...
def match_layer(weights_bias, sigma_inv_layer, mean_prior, sigma_inv_prior, gamma, it):
    ...
    print("Sorted group order is :", group_order)
...
def layer_group_descent(batch_weights, batch_frequencies, sigma_layers, sigma0_layers, gamma_layers, it):
    ...
    for c in range(1, n_layers)[::-1]:
        ...
        if c == (n_layers - 1) and n_layers > 2:
            print("we are stacking shape {} and shape {}, since n_layers>2".format(batch_weights[0][c * 2 - 1].shape, batch_weights[0][c * 2].shape))
            ...
            print("we are stacking shape {} and shape {}, since c >1".format(batch_weights[0][c * 2 - 1].shape, batch_weights[0][c * 2].shape))
