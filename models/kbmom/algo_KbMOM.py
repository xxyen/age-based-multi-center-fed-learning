...
class KbMOM:
    ...
    def inertia_function(self,idx_block,centroids = None):
        ...
        I'll replace current #159 line, so the new center would be aggregated every TRAINDED model weights(point) into one. 
        That one will be used as new centers for this block. The loss is still unsuper loss. Rest don't changes.
        '''
        if len(set(nearest_centroid)) == self.K and sum(np.bincount(nearest_centroid) > 1) == self.K :
            ...
            for k,nc in enumerate(set(nearest_centroid)):
                centers_ =  X_block[nearest_centroid == nc,:].mean(axis=0).reshape(1,-1) # will replaced by fed_train
    ...
    def medianblock_centers_function(self,X,id_median,blocks):
        ...
        '''
        Comment for David:
        X is matrix of models
        id_median is index of the median block, !It's the subset of X
        '''
